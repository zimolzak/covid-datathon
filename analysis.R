...
#  slice(1:200) %>%
  select(PAT_ID, PAT_DOB, SEX_C_ID, ORD_VALUE, IDENTITY_TYPE_ID, PRBL_ICD10, ENC_ICD10, ENC_TYPE) %>%
  mutate(result = case_when(
  	ORD_VALUE == "NEGATIVE" | ORD_VALUE == "Negative" | ORD_VALUE == "NOT DETECTED" | ORD_VALUE == "Not Detected" ~ 0,
  	ORD_VALUE == "POSITIVE" | ORD_VALUE == "Positive" | ORD_VALUE == "DETECTED"     | ORD_VALUE == "Detected" ~ 1,
